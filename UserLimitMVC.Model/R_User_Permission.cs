...
namespace UserLimitMVC.Model
{
    ...
    [KnownType(typeof(BasePermission))]
    [KnownType(typeof(BaseUser))]
    public partial class R_User_Permission: IObjectWithChangeTracker, INotifyPropertyChanged
    {
        #region 基元属性
        ...
        public int PermissionID
        {
            ...
            set
            {
                if (_permissionID != value)
                {
                    ...
                    if (!IsDeserializing)
                    {
                        if (BasePermission != null && BasePermission.ID != value)
                        {
                            BasePermission = null;
                        }
                    }
        ...
        [DataMember]
        public BasePermission BasePermission
        {
            get { return _basePermission; }
            set
            {
                if (!ReferenceEquals(_basePermission, value))
                {
                    var previousValue = _basePermission;
                    _basePermission = value;
                    FixupBasePermission(previousValue);
                    OnNavigationPropertyChanged("BasePermission");
                }
            }
        }
        private BasePermission _basePermission;
        ...
        private void FixupBasePermission(BasePermission previousValue)
        {
            if (IsDeserializing)
            {
                return;
            ...
            if (previousValue != null && previousValue.R_User_Permission.Contains(this))
            {
                previousValue.R_User_Permission.Remove(this);
            }
    
            if (BasePermission != null)
            {
                if (!BasePermission.R_User_Permission.Contains(this))
                {
                    BasePermission.R_User_Permission.Add(this);
                }
            ...
            {
                if (ChangeTracker.OriginalValues.ContainsKey("BasePermission")
                    && (ChangeTracker.OriginalValues["BasePermission"] == BasePermission))
                {
                    ChangeTracker.OriginalValues.Remove("BasePermission");
                }
                else
                {
                    ChangeTracker.RecordOriginalValue("BasePermission", previousValue);
                }
                if (BasePermission != null && !BasePermission.ChangeTracker.ChangeTrackingEnabled)
                {
                    BasePermission.StartTracking();
                }
            }
        }
        ...
        private void FixupBaseUser(BaseUser previousValue)
        {
            ...
            if (BaseUser != null)
            {
                if (!BaseUser.R_User_Permission.Contains(this))
                {
                    BaseUser.R_User_Permission.Add(this);
                }
